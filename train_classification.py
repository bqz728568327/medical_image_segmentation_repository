...
def train_net(net,
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', factor=0.5, patience=5)  # goal: maximize Dice score
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                msk = msk.to(device=device, dtype=torch.float32)

                pred_msk = net(im)


                # pred_msk = pred_msk.view_as(msk)

                # pred_arr_msk = pred_msk.cpu().detach().numpy()

                loss = criterion(pred_msk, msk)
